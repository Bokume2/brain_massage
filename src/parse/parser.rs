...
use crate::{
    ...
    parse::{AST, AssignNode, AssignNodeOperand, DynamicVariableNode, GetNode, HeadNode, LValueNode, NumberNode, PutNode, RValueNode, StatementNode, StaticVariableNode, TopLevelNode, VariableNode, WhileNode},
};
...
impl Parser {
    pub fn parse(&self, tokens: &Vec<Token>) -> Result<AST> {
    ...
    fn parse_statement(tokens: &Vec<Token>, ctx: &mut ParseContext) -> Result<StatementNode> {
        let statement_node = match tokens.get(ctx.cur).unwrap() {
            Token::Put => StatementNode::Put(Self::parse_put(tokens, ctx)?),
            Token::Variable(_) | Token::Head => StatementNode::Assign(Self::parse_assign(tokens, ctx)?),
    ...
    fn parse_while(tokens: &Vec<Token>, ctx: &mut ParseContext) -> Result<WhileNode> {
        ctx.cur += 1;
        ...
        let Token::Variable(condition_token) = Self::get_token_without_nl(tokens, ctx)? else {
            bail!(format!("expected variable, found {:?}", tokens.get(ctx.cur - 1).unwrap()));
        };
        ...
        if *Self::get_token_without_nl(tokens, ctx)? != Token::CurlyBracket(CurlyBracketToken::Close) {
            bail!("bracket mismatch, not enough \"}}\"");
        }
        Ok(WhileNode { condition, content })
    }

    fn parse_put(tokens: &Vec<Token>, ctx: &mut ParseContext) -> Result<PutNode> {
        ctx.cur += 1;
        if *Self::get_token_without_nl(tokens, ctx)? != Token::Bracket(BracketToken::Open) {
    ...
    fn parse_get(tokens: &Vec<Token>, ctx: &mut ParseContext) -> Result<GetNode> {
        ctx.cur += 1;
        if *Self::get_token_without_nl(tokens, ctx)? != Token::Bracket(BracketToken::Open) {
    ...
    fn parse_assign(tokens: &Vec<Token>, ctx: &mut ParseContext) -> Result<AssignNode> {
        ...
        let Token::Assign(assign_token) = Self::get_token_without_nl(tokens, ctx)? else {
            bail!("expected assignment, found {:?}", tokens.get(ctx.cur - 1).unwrap())
        };
    ...
    fn parse_variable(var_token: &VariableToken) -> VariableNode {
        match var_token {
            VariableToken::Static { index } => VariableNode::Static(StaticVariableNode{ index:*index}),
            VariableToken::Dynamic => VariableNode::Dynamic(DynamicVariableNode),
        }
    }

    fn get_token_without_nl<'a>(tokens: &'a Vec<Token>, ctx: &mut ParseContext) -> Result<&'a Token> {
        loop {
